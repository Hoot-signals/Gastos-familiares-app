...
- **Deployment gotcha (cost an afternoon on 2026-09-07)**: the Apps Script project is *standalone* — a loose file in Drive, not bound to the Sheet — so deleting it, or archiving its Web App deployment, kills the app while the Sheet keeps opening normally; "Sheets works" says nothing about the backend. After editing the backend always redeploy via *Implementar → Gestionar implementaciones → lápiz → Nueva versión*, which keeps the same `/exec` URL. "Nueva implementación" mints a **new** URL and the old one starts answering `El archivo solicitado no existe` once retired — and since each phone keeps its own copy of the URL in Ajustes, a phone left on the old one goes dark by itself (that is exactly what happened: two `/exec` URLs that looked identical by eye). Update every phone before retiring a deployment. Ajustes → **Probar conexión** diagnoses this: it runs the real GET and prints the exact server error plus a signature (URL tail + a SHA-256 fingerprint of the secret) that can be compared between phones without revealing the token.
